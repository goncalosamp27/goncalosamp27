...
<section
  ...
  style="background: linear-gradient(var(--ocean-blue) 0%, var(--ocean-blue) 40%, var(--ocean-yellow) 100%); 
        color: var(--white)"
>

{#if show}
<header class="w-full px-12 py-8 relative flex items-center justify-between">
    <div class="w-12 h-12 text-[var(--white)] hover:text-[var(--ocean-yellow)] transition-colors duration-300 cursor-pointer">
    ...
    <nav class="flex justify-center space-x-35 text-2xl font-regular">
        <a href="#about"
            class="min-w-[10ch] text-center rounded-full px-4 py-2 
                    ...
                    hover:border-[var(--ocean-yellow)] 
                    hover:text-[var(--ocean-hover)]">
            About Me
        </a>
        <a href="#projects"
            class="min-w-[10ch] text-center rounded-full px-4 py-2 
