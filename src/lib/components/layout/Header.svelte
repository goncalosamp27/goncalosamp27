<script lang="ts"> 
  ...
  function sectionHref(id: string) {
    return standalone ? `${homeHref}#${id}` : `#${id}`;
  }
  ...
  function spinIn(node: HTMLElement, { duration = 800, delay = 250 } = {}) {
    return {
      duration,
      delay,
      easing: cubicOut,
      css: (t: number) => `
        opacity: ${t};
        transform: rotate(${(1 - t) * 180}deg) scale(${0.4 + t * 0.6});
      `
    };
  }

  function spinOut(node: HTMLElement, { duration = 400 } = {}) {
    return {
      duration,
      easing: cubicIn,
      css: (t: number) => `
...
<header class="w-full -mt-2 md:px-12  py-8 fixed top-0 left-0 flex items-center justify-between bg-gradient-to-b from-black/70 to-black/0 z-[6]" in:fade={{ duration: 1500 }}>
    <a href={standalone ? homeHref : '#hero'} on:click={(event) => handleSectionClick(event, 'hero')} class="hidden md:block w-12 h-12 text-[var(--white)] hover:text-[var(--hover)] transition-colors duration-300 cursor-pointer" aria-label = "logo">
    ...
    <nav class="hidden md:flex justify-center space-x-4 md:space-x-10 text-2xl font-regular">
        <a href={sectionHref('about')}
            on:click={(event) => handleSectionClick(event, 'about')}
        ...
        <a href={sectionHref('projects')}
            on:click={(event) => handleSectionClick(event, 'projects')}
            class="min-w-[10ch] text-center rounded-full px-4 py-2 
                    border-2 border-transparent
                    transition-all duration-500
                    hover:border-[var(--hover)] 
        ...
        <a href={sectionHref('experience')}
            on:click={(event) => handleSectionClick(event, 'experience')}
            class="min-w-[10ch] text-center rounded-full px-4 py-2 
                    border-2 border-transparent
                    transition-all duration-500
                    hover:border-[var(--hover)] 
    ...
    <!-- Theme Menu -->
    <div class="relative mr-4 md:mr-0" on:mouseleave={closeThemeMenu} role="group" aria-label="Theme selector">
